...
def guidance_setup(guidance_opt):
    if guidance_opt.guidance == "SD":
        ...
        guidance = StableDiffusion(guidance_opt.g_device,
                                   guidance_opt.fp16,
                                   guidance_opt.vram_O,
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations,
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if guidance_opt.perpneg:
            loss = guidance.train_step_perpneg(torch.stack(text_z_, dim=1),
                                               images,
                                               pred_depth=depths,
                                               pred_alpha=alphas,
            ...
            loss = guidance.train_step(torch.stack(text_z_, dim=1),
                                       images,
                                       pred_depth=depths,
                                       pred_alpha=alphas,
                                       grad_scale=guidance_opt.lambda_guidance,
    ...
    if iteration in testing_iterations:
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            for idx, viewpoint in enumerate(config['cameras']):
                ...
                if tb_writer:
                    tb_writer.add_images(config['name'] + "_view_{}/render".format(viewpoint.uid),
                                         image[None],
...
def video_inference(iteration, scene: Scene, renderFunc, renderArgs):
    ...
    if config['cameras'] and len(config['cameras']) > 0:
        ...
        if len(depth_frames) > 0:
            imageio.mimwrite(os.path.join(save_folder, "video_depth_{}.mp4".format(iteration)),
                             depth_frames,
                             fps=30,
                             quality=8)
